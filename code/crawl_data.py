...
def crawl_ten_thousand_page_of_meey(start_id: int, file_name: str):
    urls = [
        f"https://apisuperapp.meeyland.com/v1/articles/{listing_id}" 
        for listing_id in range(start_id, start_id + 10000)
    ]
    cnt = 0
    with open(file_name, 'a', encoding='utf-8') as f:
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            futures = {executor.submit(crawl_a_meey, url): url for url in urls}
